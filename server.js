const express = require('express'); // Expressをインポート(Webサーバー用)
const cron = require('node-cron'); // cronをインポート(スケジュール管理用)
const {
  app,
  fetchChannelMembers,
  fetchUserInfo,
  ...
  consecutiveDays,
  shuffle
} = require('./index'); // index.jsから必要なモジュールをインポート

const serverApp = express(); // Expressアプリケーションを作成
const port = process.env.PORT || 3000; // ポート番号を指定

// Glitchのサーバーがスリープから復帰するためのエンドポイント
serverApp.get('/', (req, res) => {
  console.log("Get request")
  res.send('Glitch woke up'); // サーバーが起動していることを確認するためのレスポンス
});
...
cron.schedule('44 14 * * 3', async () => {
  ...
  const memberIds = await fetchChannelMembers(channelId); // チャンネルメンバーを取得

  store.clearUsers(); // ユーザー情報をクリア

  const botId = await app.client.auth.test({
    token: process.env.SLACK_BOT_TOKEN
  }).then(response => response.user_id); // BotのIDを取得

  for (const memberId of memberIds) {
    if (memberId === botId) continue; // 自分自身のBotを除外

    const user = await fetchUserInfo(memberId); // ユーザー情報を取得
  ...
  const members = store.getUsers(); // ユーザー情報を取得
  const groupAMembers = Object.values(members).filter(user => user.group === 'A'); // グループAのメンバーを取得
  const groupBMembers = Object.values(members).filter(user => user.group === 'B'); // グループBのメンバーを取得

  // 掃除の振り分け
  assignedTasks = [];  // assignedTasksをクリア
  const is_AGroup = (WEEK_NUMBER % 2 === 0) // 隔週判定
  const cleaningLocations = shuffle(locations.getLocations()); // 掃除場所をシャッフル
  let messageText = ''; // メッセージテキストを初期化

  // 前回の掃除をしていないメンバーをメンション
  const incompleteTasks = assignedTasks.filter(task => !task.completed); // 前回の掃除をしていないメンバーをフィルタリング
