const { App } = require('@slack/bolt'); // Slack APIを使用するためのBoltライブラリをインポート
const store = require('./store'); // store.jsをインポート
const groups = require('./groups'); // groups.jsをインポート
const locations = require('./locations'); // locations.jsをインポート
const thanksMessage = require('./message'); // message.jsをインポート
const { saveData, loadData } = require('./saveData'); // saveData.jsをインポート
...
(async () => {
  ...
  try {
    ...
    console.error("Failed to start Bold app:", error);
  }
})();
...
async function performScheduledTasks() {
  ...
  console.log("A: ", groupAMembers);
  console.log("B:", groupBMembers);
